...
import { useContasReceber, useMarcarContaRecebida, useDeleteContaReceber, ContaReceber } from "@/hooks/use-financeiro";
import { useClientes } from "@/hooks/use-clientes";
import { ContaReceberForm } from "@/components/forms/conta-receber-form";
...
export default function ContasReceber() {
  ...
  const { data: contas, isLoading, error } = useContasReceber();
  const { data: clientes } = useClientes();
  const marcarRecebida = useMarcarContaRecebida();
  ...
  const handleMarcarRecebida = async (id: string) => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Erro ao marcar conta como recebida. Tente novamente.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeleteConta = (conta: ContaReceber) => {
    setSelectedConta(conta);
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            {row.status !== "paga" && (
              <>
                ...
                <DropdownMenuItem onClick={() => handleMarcarRecebida(row.id)}>
      ...
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Gerencie todas as contas a receber de clientes
          </p>
        </div>
        ...
        <DataTable
          ...
          data={contas || []}
          columns={columns}
          searchPlaceholder="Pesquisar por cliente, descrição..."
          filters={filters}
          actions={
          ...
          onExportCSV={() => {
            toast({
              title: "Exportando contas a receber",
        ...
        <ContaDetailsDialog
          ...
          conta={selectedConta}
          type="receber"
          clienteNome={selectedConta?.cliente?.nome || getClienteNome(selectedConta?.cliente_id || '')}
        />
